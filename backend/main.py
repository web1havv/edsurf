...
@app.post("/generate-case-study-async")
async def generate_case_study_async(file: UploadFile = File(...), speaker_pair: str = Form(None)):
    """Generate case study video asynchronously - returns job ID immediately"""
    request_id = f"casestudy_async_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    logger.info(f"🔄 [{request_id}] Starting async case study processing from file: {file.filename}")
    ...
    try:
        ...
        try:
            # Create async job
            job_id = async_processor.create_job(
                task_type='case_study_video',
                parameters={
            ...
            logger.info(f"✅ [{request_id}] Created async job: {job_id}")
            
            return {
                "job_id": job_id,
                "status": "pending",
                "message": "Video generation started. Use /job-status/{job_id} to check progress.",
        ...
        logger.error(f"❌ [{request_id}] Error during async case study processing: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/generate-case-study-text-async")
async def generate_case_study_text_async(request: CaseStudyTextRequest):
    """Generate case study video asynchronously from text - returns job ID immediately"""
    request_id = f"casestudy_text_async_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    logger.info(f"🔄 [{request_id}] Starting async case study processing from text")
    ...
    try:
        ...
        logger.error(f"❌ [{request_id}] Error during async case study text processing: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def list_active_jobs():
    ...
    logger.info("📋 Listing active jobs")
    
    # Clean up old jobs first
    async_processor.cleanup_old_jobs()
    
    active_jobs = []
    for job_id, job in async_processor.jobs.items():
        job_info = {
            "job_id": job_id,
            "status": job.status,
        ...
        if job.started_at:
            job_info["started_at"] = job.started_at.isoformat()
        if job.completed_at:
    ...
    return {
        ...
        "active_jobs": len([j for j in active_jobs if j["status"] in ["pending", "processing"]])
    }
...
async def cancel_job(job_id: str):
    ...
    if job_id not in async_processor.jobs:
        raise HTTPException(status_code=404, detail="Job not found")
    
    job = async_processor.jobs[job_id]
    ...
    # Remove from processing threads if active
    if job_id in async_processor.processing_threads:
        del async_processor.processing_threads[job_id]
